...
(
    ...
    CONSTRAINT CK_titulo_rev CHECK (titulo ~ '^[A-Z]'),
...
(
    ...
    CONSTRAINT CK_apellidos_emp1 CHECK (apellidos ~ '^[A-Z]'),
    ...
    CONSTRAINT CK_nacionalidad_emp CHECK (nacionalidad ~ '[A-Z]')
);
...
(
    ...
    CONSTRAINT CK_nombre_peri CHECK (nombre ~ '^[A-Z]'),
    CONSTRAINT CK_apellidos_peri CHECK (apellidos ~ '^[A-Z]'),
...
-- 5
ALTER TABLE NumRevista ADD CONSTRAINT CK_ISSN_numrev CHECK (ISSN ~ '[1-9]{0,3}-[1-9]{0,3}');
-- 6
...
-- 9
ALTER TABLE Empleado ADD CONSTRAINT CK_DNI_emp CHECK (DNI_Empleado ~ '[1-9]{0,7}[A-Z]$');
ALTER TABLE Periodista ADD CONSTRAINT CK_DNI_peri CHECK (DNI_Periodista ~ '[1-9]{0,7}[A-Z]$');
-- 10
